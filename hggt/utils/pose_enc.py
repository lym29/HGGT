...
def extri_intri_to_pose_encoding(
    extrinsics, intrinsics=None, image_size_hw=None, pose_encoding_type="absT_quaR_FoV"  # e.g., (256, 512)
):
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        if intrinsics is not None:
            ...
            # Pad with zeros when intrinsics are not available
            zeros_h = torch.zeros_like(T[..., :1])  # BxSx1
            zeros_w = torch.zeros_like(T[..., :1])  # BxSx1
            pose_encoding = torch.cat([T, quat, zeros_h, zeros_w], dim=-1).float()
    else:
